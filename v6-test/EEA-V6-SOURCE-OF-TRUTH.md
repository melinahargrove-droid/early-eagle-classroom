...
- Student fallback rosters are inconsistent across older files.
- App Settings contains options/copy that do not consistently match current behavior.
- Teacher-only management pages need a shared access guard.
- Week 8 Friday Storytelling needs to use child storytelling.
- Older lesson screens contain obsolete `weekX-plan.html` and `daily-lessons-v2.html` navigation.
