(()=>{
  ...
  function savedRoster(){
    try{const saved=JSON.parse(localStorage.getItem(STUDENT_KEY)||'[]');if(Array.isArray(saved))return saved}catch(e){}
    return [];
  }
  ...
  function syncInlineRoster(){
    ...
    try{if(typeof students!=='undefined'&&Array.isArray(students))students.splice(0,students.length,...roster.map((s,i)=>({id:String(s.id||('s'+i)),name:String(s.name).trim(),photo:s.photo||'',audio:s.audio||''})))}catch(e){}
  }
  ...
  function safePresentIds(){
    try{
      ...
      return [...new Set(raw.map(v=>{let id=String(v);if(/^\d+$/.test(id)&&list[Number(id)])id=String(list[Number(id)].id);return id}).filter(id=>known.has(id)))];
    }catch(e){return []}
  }
  ...
  function seedFreshStarFirstRound(){
    try{
      ...
      counts.fill(0);
      selectedFriendId='';selectionLocked=false;chooserStarted=false;transitionPending=false;
  ...
  function restoreState(){
    try{
      ...
      if(!hadMeaningfulProgress){seedFreshStarFirstRound();return true}
      centerHistory.splice(0,centerHistory.length,...history);counts.fill(0);
      ...
      selectedFriendId=selected;selectionLocked=Boolean(saved.selectionLocked)&&Boolean(selected);chooserStarted=eligible.size>0&&(Boolean(saved.chooserStarted)||history.length>0||Boolean(selected)||friendQueue.length>0);transitionPending=false;
      if(typeof renderCenters==='function')renderCenters();
      if(selected&&typeof showFriend==='function')showFriend(selected);else{const name=document.getElementById('friendName'),personEl=document.getElementById('person'),photo=document.getElementById('friendPhoto');if(name)name.textContent='';if(personEl)personEl.classList.remove('show-photo');if(photo)photo.innerHTML=''}
      return true;
  ...
  function showEmptyState(){
    const old=document.getElementById('eea-center-empty-state');if(old)old.remove();
    const roster=activeRoster(),present=safePresentIds();if(roster.length&&present.length)return false;
    ...
    const stage=document.querySelector('.stage');if(!stage)return true;
    const veil=document.createElement('div');veil.id='eea-center-empty-state';veil.style.cssText='position:absolute;inset:15vh 18vw 12vh;z-index:40;border:2px solid #8ca9bf;border-radius:30px;background:rgba(255,250,242,.97);box-shadow:0 18px 42px rgba(35,76,119,.15);display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:34px;color:#173f72;';
    const heading=document.createElement('div');heading.style.cssText='font-size:clamp(30px,2.6vw,50px);font-weight:900;margin-bottom:14px;';
    const detail=document.createElement('div');detail.style.cssText='font-size:clamp(17px,1.35vw,25px);font-weight:700;line-height:1.35;max-width:660px;margin-bottom:24px;';
    const action=document.createElement('button');action.type='button';action.style.cssText='border:1.5px solid rgba(106,149,181,.6);border-radius:17px;background:#fffaf2;color:#173f72;font-size:clamp(18px,1.35vw,25px);font-weight:900;padding:13px 24px;cursor:pointer;';
    ...
    veil.append(heading,detail,action);stage.appendChild(veil);['pickBtn','timerBtn','undoBtn','resetBtn'].forEach(id=>{const el=document.getElementById(id);if(el)el.disabled=true});
