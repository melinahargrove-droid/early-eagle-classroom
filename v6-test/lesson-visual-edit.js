(()=>{
  ...
  const selectors=['#visual','#art','.lesson-visual','.storyvis','.closevis','.scene','.visual'];
  let renderToken=0;
  let observerTimer=0;
  ...
  function targets(){
    ...
    return out.filter((el,i,all)=>!all.some((other,j)=>j!==i&&other.contains(el)&&selectors.some(s=>other.matches(s))));
  }
  ...
  async function ensureStore(){
    ...
    if(!script){
      ...
      script.id='eeaVisualStoreForLessonEdit';
      script.src='visual-store.js';
  ...
  async function decorate(el,index,token){
    ...
    if(!btn){
      ...
      btn.type='button';
      btn.className='eea-visual-edit-btn';
      btn.dataset.eeaEditChrome='1';
      btn.textContent='✎ Edit Visual';
      el.appendChild(btn);
    }
    btn.dataset.slot=slot;
    btn.onclick=event=>{
      event.preventDefault();event.stopPropagation();
      location.href='visual-editor.html?slot='+encodeURIComponent(slot)+'&return='+encodeURIComponent(returnUrl());
    };

    let custom=el.querySelector(':scope > .eea-custom-visual');
    if(!custom){
      custom=document.createElement('img');
      custom.className='eea-custom-visual';
      custom.alt='Custom lesson visual';
      custom.hidden=true;
      el.insertBefore(custom,btn);
    }
    custom.hidden=true;
    if(!window.EEAVisualStore)return;
    try{
  ...
  async function apply(){
    ...
    const list=targets();
    await Promise.all(list.map((el,index)=>decorate(el,index,token)));
  }
  ...
  document.addEventListener('click',event=>{
    if(event.target.closest('.eea-visual-edit-btn'))return;
