...
(function(){
'use strict';
const STUDENT_KEY='eea-students-v1',ATTENDANCE_KEY='eea-attendance-present',CALENDAR_KEY='eea-school-calendar',STAR_KEY='eea-current-star',STATE_KEY='eea-star-state-v1',REVEAL_KEY='eea-star-revealed-date';
function dateKey(d=new Date()){return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0')}
function roster(){let a=[];try{const x=JSON.parse(localStorage.getItem(STUDENT_KEY)||'[]');if(Array.isArray(x))a=x.filter(s=>s&&s.active!==false&&String(s.name||'').trim()).map((s,i)=>({id:String(s.id||('s'+i)),name:String(s.name).trim()}))}catch(e){}a.sort((x,y)=>x.name.localeCompare(y.name));return a}
...
function overrideById(id){const r=roster(),c=r.find(x=>x.id===id);if(!c)return load();let s=load();resetCycleIfNeeded(s);const today=dateKey(new Date()),previousId=s.lastResolvedDate===today?String(s.currentId||''):'';if(previousId&&previousId!==String(id))s.servedIds=s.servedIds.filter(x=>String(x)!==previousId);if(!s.servedIds.includes(id))s.servedIds.push(id);s.pendingIds=s.pendingIds.filter(x=>x!==id);s.currentId=id;s.currentName=c.name;s.lastResolvedDate=today;s.nextIndex=firstUnservedIndex(s,0);if(previousId&&previousId!==String(id))localStorage.removeItem(REVEAL_KEY);return save(s)}
