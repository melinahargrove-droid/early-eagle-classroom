...
(function(){
'use strict';
const STUDENT_KEY='eea-students-v1',ATTENDANCE_KEY='eea-attendance-present',ATTENDANCE_DATE_KEY='eea-attendance-date',CALENDAR_KEY='eea-school-calendar',STAR_KEY='eea-current-star',STATE_KEY='eea-star-state-v1';
...
function attendanceTakenToday(){return localStorage.getItem(ATTENDANCE_DATE_KEY)===dateKey()}
function presentIds(){const r=roster(),byId=new Map(r.map(s=>[s.id,s]));if(!attendanceTakenToday())return [];try{const x=JSON.parse(localStorage.getItem(ATTENDANCE_KEY)||'[]');if(!Array.isArray(x))return [];return [...new Set(x.map(v=>String(v)).filter(id=>byId.has(id)))]}catch(e){return []}}
...
function current(){const r=roster(),s=load(),c=r.find(x=>x.id===s.currentId);if(c)return c;const byName=r.find(x=>x.name===localStorage.getItem(STAR_KEY));return byName||null}
