...
const LESSON_FLOW_MIGRATION='eea-lesson-flow-schema-v2';
const CLASS_KEYS=[
  ...
  'eea-lesson-resume','eea-lesson-auto-resume'
];
const CLASS_KEY_PATTERNS=[/^eea-complete-w\d+-d\d+$/];
function isClassKey(key){return CLASS_KEYS.includes(key)||CLASS_KEY_PATTERNS.some(pattern=>pattern.test(String(key||'')))}
function normalize(v){return String(v||'').toLowerCase()==='pm'?'pm':'am'}
function localDateKey(d=new Date()){return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0')}
function sanitizeAttendanceState(){
  try{
    const stamp=localStorage.getItem('eea-attendance-date')||'';
    if(stamp===localDateKey())return false;
    localStorage.setItem('eea-attendance-present','[]');
    localStorage.setItem('eea-attendance-count','0');
    localStorage.removeItem('eea-attendance-date');
    return true;
...
function ensureRosterState(){
  let roster=[];
  try{
    const parsed=JSON.parse(localStorage.getItem('eea-students-v1')||'[]');
    if(Array.isArray(parsed))roster=parsed;
  }catch(e){}
  localStorage.setItem('eea-students-v1',JSON.stringify(roster));
  const activeNames=roster.filter(s=>s&&s.active!==false&&String(s.name||'').trim()).map(s=>String(s.name).trim());
  localStorage.setItem('eea-student-names',JSON.stringify(activeNames));
  return roster;
}
...
function liveClassKeys(){const keys=new Set(CLASS_KEYS);for(let i=0;i<localStorage.length;i++){const key=localStorage.key(i);if(isClassKey(key))keys.add(key)}return [...keys]}
function capture(){const data={};liveClassKeys().forEach(k=>{const v=localStorage.getItem(k);if(v!==null)data[k]=v});return data}
function save(which=active()){ensureRosterState();localStorage.setItem(profileKey(which),JSON.stringify(capture()));return true}
function clearLive(){liveClassKeys().forEach(k=>localStorage.removeItem(k))}
function restore(which){const target=normalize(which);clearLive();let data={};try{data=JSON.parse(localStorage.getItem(profileKey(target))||'{}')||{}}catch(e){data={}};Object.keys(data).forEach(k=>{if(isClassKey(k)&&data[k]!==null&&data[k]!==undefined)localStorage.setItem(k,String(data[k]))});ensureRosterState();sanitizeAttendanceState();localStorage.setItem(ACTIVE_KEY,target);return target}
...
window.EEAClassProfile={active,switchTo,save,restore,label,keys:CLASS_KEYS.slice(),isClassKey,sanitizeAttendanceState};
active();ensureRosterState();sanitizeAttendanceState();migrateLessonFlowOnce();migrateAppSettings();
